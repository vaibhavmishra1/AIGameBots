...
def normalize_targets(actions: torch.Tensor) -> torch.Tensor:
    # Model outputs 5 pairs of (dx,dy) in [-1,1] for next 5 timesteps.
...
def evaluate(model, loader, device, experiment_type, huber: bool = False) -> float:
    ...
    if experiment_type == "hawk_temporal_and_spatial":
        ...
        for temporal, actions in tqdm(loader, desc="Val", leave=False):
            ...
            pred = model(temporal)
            #target = sanitize_tensor(y, name="target")
            target = y
            loss = criterion(pred, target)
            bs = temporal.size(0)
            total_loss += loss.item() * bs
    ...
    if experiment_type == "hawk_temporal_and_spatial":
        for temporal, spatial, actions in loader:
            ...
            for i in range(temporal.size(0)):
                ...
                action_list = actions[i].tolist()
                # Print first timestep as example
                px, py = pred_list[0], pred_list[1]
                ax, ay = action_list[0], action_list[1]
                print(f"[{shown:02d}] t0: tgt=({ax:.4f},{ay:.4f}) pred=({px:.4f},{py:.4f})")
                shown += 1
                if shown >= k:
                    return
    elif experiment_type == "hawk_temporal_only":
...
python3 train.py \
  ...
  --lr 2e-4 --num_workers 4 --huber --experiment_type hawk_temporal_only > output_temporal_only.txt 


python3 train.py \
  --h5_path "dataset_exp_hawk_0p02_0p3_100000.h5" \
  --epochs 20 --batch_size 128 \
  --lr 2e-4 --num_workers 4 --huber --experiment_type hawk_spatial_only > output_spatial_only.txt 


python3 train.py \
  --h5_path "dataset_exp_hawk_0p02_0p3_100000.h5" \
  --epochs 20 --batch_size 128 \
  --lr 2e-4 --num_workers 4 --huber --experiment_type hawk_only_main_agent_current_features > output_only_main_agent_current_features.txt 

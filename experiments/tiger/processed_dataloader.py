...
class ProcessedH5Dataset(Dataset):
    ...
    def __init__(self, h5_path, experiment_type = "tiger_temporal_and_spatial",group_name="processed", return_numpy=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.experiment_type == "tiger_temporal_and_spatial":
            return (
        ...
        elif self.experiment_type == "tiger_temporal_only":
            return (
            torch.from_numpy(np.asarray(temporal, dtype=np.float32)),
            torch.from_numpy(np.asarray(actions, dtype=np.float32)),
            )
        elif self.experiment_type == "tiger_spatial_only":
            return (
            torch.from_numpy(np.asarray(spatial, dtype=np.float32)),
            torch.from_numpy(np.asarray(actions, dtype=np.float32)),
            )
        elif self.experiment_type == "tiger_only_main_agent_current_features":
            temporal = temporal[-1]
            return (
            torch.from_numpy(np.asarray(temporal, dtype=np.float32)),
            torch.from_numpy(np.asarray(actions, dtype=np.float32)),
...
def main():
    h5_path = "/Users/vaibhav/Desktop/AIGameBots/experiments/tiger/dataset_exp_tiger_0p02_0p3_100000.h5"
    experiment_type = "tiger_only_main_agent_current_features"

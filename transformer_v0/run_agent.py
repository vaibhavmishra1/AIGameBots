...
# Ensure we can import shared CSGO modules (config, screen_input, key I/O)
...
# PyTorch model imports (Transformer ViViT)
import torch
from model import create_vivit_model

# Local action dimension for aux construction
...
class TorchAgent:
    """
    Wrapper around the ViViT Transformer model to match the minimal interface used by the TF agent:
    - predict_on_batch(np_batch) -> np.ndarray
    - reset_states() -> None
    """

    def __init__(self, checkpoint_path: str, model_name: str = 'vivit_vitb16', aux_input_on: bool = True, stateful: bool = True):
        self.device = get_device()
        self.model = create_vivit_model(model_name=model_name, aux_input_on=aux_input_on)
        self.model.to(self.device)
        self.model.eval()
        # enable stateful sequence processing if requested (no-op for this model)
        try:
    ...
    @torch.no_grad()
    def predict_on_sequence(self, x_seq_np: np.ndarray, aux_seq_np: np.ndarray) -> np.ndarray:
        # Expect shapes: x_seq (1, T, H, W, 3), aux_seq (1, T, ACTION_DIM)
        x = torch.from_numpy(x_seq_np).float()
        if x.max() > 1.0:
            x = x / 255.0
        x = x.to(self.device)

        aux = torch.from_numpy(aux_seq_np).float().to(self.device)
        ...
        y = torch.cat([keys, clicks, mouse_x, mouse_y, value], dim=-1)
        return y.detach().cpu().numpy()
    ...
    def predict_with_dynamic_k(self, recent_imgs: list, recent_actions: list) -> tuple:
        ...
        aux_seq = np.zeros((1, k, ACTION_DIM), dtype=np.float32)
        for t_i in range(1, k):
            aux_seq[0, t_i, :] = recent_actions[-k + t_i - 1]

        y = self.predict_on_sequence(x_seq, aux_seq)
        return y, k
...
def onehot_to_actions(y_preds):
    # Local version that also returns value prediction as 6th element
...
# this script applies a trained Transformer NN in a deathmatch environment
...
# load model names (kept for stats/printing consistency)
model_names = ['vivit_vitb16_transformer_infer']
...
for training_iter in range(n_iters_total):
    ...
    # context length for Transformer temporal attention
    context_len = int(os.environ.get('TRANSFORMER_CONTEXT', str(N_TIMESTEPS)))
    for i in range(0, context_len):
        ...
        # actions
        dummy_action = np.zeros(int(ACTION_DIM))
    ...
    # Load Transformer model (kept outside TF loader)
    checkpoint_dir = os.path.join(os.path.dirname(__file__), 'checkpoints')
    # Default to best checkpoint name from transformer training script
    default_ckpt = os.path.join(checkpoint_dir, 'vivit_vitb16_best_iter2.pt')
    checkpoint_path = os.environ.get('TRANSFORMER_AGENT_CKPT', default_ckpt)
    print('loading Transformer model from', checkpoint_path)
    model_run = TorchAgent(checkpoint_path=checkpoint_path, model_name='vivit_vitb16', aux_input_on=True)
    ...
    while n_loops < 1000 * (mins_per_iter + 0.02):  # x minutes
        ...
        # Dynamic-k sequence selection: compute k from current frame, then run on last k
        ...
        # overwrite best guess action with probabilistically selected action
        ...
        helper_i = np.zeros(6)  # this is now [0 kill, 1 death, 2 original_value_pred, 3 advantage, 4 reward_t, 5 value_t ]
        helper_i[2] = val_pred
        curr_vars = {}
        if IS_GSI:
        ...
        # refresh GSI - less important to be up to date, so do last
        if IS_GSI:
            server.handle_request()
        ...
        # mouse movement smoothing (split move)

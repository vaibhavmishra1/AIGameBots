...
class AgentControlDataset(Dataset):
    ...
    def preprocess_data(self, data):
        ...
        if data.ndim != 3:
            ...
            return empty_spatial, zero_actions
        ...
        for j in range(num_agents):
            feature = data[timesteps_total - 2][j]
            if np.all(feature == 0):
                # If last timestep and agent 0 is all zeros, still keep actions as zeros
                continue

            prev_feature = data[timesteps_total - 3][j] 
        ...
        return new_spatial_data, actions
    ...
    def __getitem__(self, idx):
        ...
        spatial, actions = self.preprocess_data(data)
...
def export_processed_to_hdf5(dataset, output_h5_path, group_name="processed", compression="gzip", compression_level=4):
    ...
    with h5py.File(output_h5_path, 'a') as out_f:
        ...
        for index in tqdm(range(len(dataset)), desc="Exporting samples to HDF5"):
            ...
            if (dx * dx + dy * dy) > 0.1 and (dx * dx + dy * dy) < 0.3: 
...
def main():
    ...
    print(len(dataset))
    spatial, actions = dataset[10]
    print(spatial.shape, actions.shape)
    ...
    start =  time.time()
    output_h5 = "/Users/vaibhav/Desktop/processed_game_logs_attention_spatial_only_sub_0_1_0_3_100000.h5"

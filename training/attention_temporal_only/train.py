...
def train_one_epoch(model, loader, optimizer, device, huber: bool = False) -> float:
    model.train()
...
@torch.no_grad()
def evaluate(model, loader, device, huber: bool = False) -> float:
    model.eval()
    criterion = nn.SmoothL1Loss() if huber else nn.MSELoss()
    total_loss = 0.0
    total_count = 0
...
def main():
    ...
    parser.add_argument("--h5_path", type=str, default="/Users/vaibhav/Desktop/processed_game_logs_attention_temporal_only_sub_0_1_0_3_100000.h5")
    ...
    parser.add_argument("--temp_layers", type=int, default=2, help="Number of transformer layers")
    parser.add_argument("--temp_heads", type=int, default=8, help="Number of attention heads")
    ...
    model = build_model(
        feature_dim=5,  # Using 5 features now
        d_model=args.d_model,
        num_layers=args.temp_layers,
        num_heads=args.temp_heads,
        dropout=args.dropout,
        max_time=20,  # Fixed sequence length of 20
    ).to(device)
    ...
    for epoch in tqdm(range(start_epoch, args.epochs + 1), desc="Epochs"):
        train_loss = train_one_epoch(model, train_loader, optimizer, device,  huber=args.huber)
        val_loss = evaluate(model, val_loader, device,  huber=args.huber)

...
class AgentControlDataset(Dataset):
    def __init__(self, dataset_path, slice_fn=None, return_numpy=False, debug=False):
        ...
        self.min_feature_values = [float('inf')] * 5
        self.max_feature_values = [float('-inf')] * 5
...
#                 deltay, #11
        feature_names = [
            'rel_pos_x', 'rel_pos_z', 'shrinking_key', 'delta_x', 'delta_y'
        ]

        temporal_values = [[] for _ in range(5)]
        spatial_values = [[] for _ in range(5)]
        action_values = [[] for _ in range(2)]
        ...
        for idx in tqdm(random_indices):
            ...
            # try:
            #     dx, dy= float(actions[0]), float(actions[1])
    ...
    def preprocess_data(self, data):
        ...
        for i in range(timesteps_total):
            ...
            prev_feature = data[i][0] if i == 0 else data[i - 1][0]
        ...
        return new_temporal_data, actions
    ...
    def __getitem__(self, idx):
        ...
        temporal, actions = self.preprocess_data(data)
...
def export_processed_to_hdf5(dataset, output_h5_path, group_name="processed", compression="gzip", compression_level=4):
    ...
    with h5py.File(output_h5_path, 'a') as out_f:
        ...
        for index in tqdm(range(len(dataset)), desc="Exporting samples to HDF5"):
            ...
            temporal,  actions = dataset[index]
            if isinstance(temporal, torch.Tensor):
            ...
            #     skipped += 1
            #     continue
            dx = float(actions_np[0])
            dy = float(actions_np[1])
            if (dx * dx + dy * dy) > 0.02 and (dx * dx + dy * dy) < 0.3: 
...
def main():
    ...
    print(len(dataset))
    temporal, actions = dataset[10]
    print(temporal.shape,  actions.shape)
    ...
    start =  time.time()
    output_h5 = "/Users/vaibhav/Desktop/processed_game_logs_attention_temporal_only_sub_0_02_0_3_all.h5"

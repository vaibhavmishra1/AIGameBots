...
class SpatialSetEncoder(nn.Module):
    ...
    def __init__(self, feature_dim: int = 13, d_model: int = 256, nhead: int = 8, num_layers: int = 2, dropout: float = 0.1):
        super().__init__()
...
class TemporalEncoder(nn.Module):
    def __init__(self, feature_dim: int = 13, d_model: int = 256, nhead: int = 8, num_layers: int = 2, dropout: float = 0.1, max_time: int = 64):
        super().__init__()
        self.embed = nn.Sequential(
            nn.Linear(feature_dim, d_model),
            nn.GELU(),
...
class TemporalSpatialTransformer(nn.Module):
    def __init__(self, feature_dim: int = 13, d_model: int = 256, temp_layers: int = 2, temp_heads: int = 8, spat_layers: int = 2, spat_heads: int = 8, dropout: float = 0.1, max_time: int = 64):
        ...
        self.fuse = nn.Sequential(nn.Linear(d_model * 2, d_model), nn.GELU(), nn.Dropout(dropout))
        self.head = nn.Sequential(nn.Linear(d_model, d_model), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model, 3))
    ...
    def forward(self, temporal: torch.Tensor, spatial: torch.Tensor) -> torch.Tensor:
        ...
        out = self.head(h)  # (B,3)
        # scale: dx,dy -> [-1,1] ; rot -> [-30,30]
        scales = torch.tensor([1.0, 1.0, 30.0], device=out.device, dtype=out.dtype).unsqueeze(0)
